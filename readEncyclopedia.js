...
const workbook = XLSX.readFile('./rf4_encyclopedia.xlsx');
...
const rodEquipment = rodData.map(item => ({
...
const reelEquipment = reelData.map(item => ({
  equipmentName: item['条目名称'] || '',
  equipmentType: '渔轮',
  category: item['大类'] || '',
  subCategory: item['小类'] || '',
...
fs.writeFileSync(path.join(__dirname, 'public', 'rod_compare.json'), JSON.stringify(rodEquipment, null, 2));
console.log(`鱼竿数据导出成功: ${rodEquipment.length} 条`);

fs.writeFileSync(path.join(__dirname, 'public', 'reel_compare.json'), JSON.stringify(reelEquipment, null, 2));
